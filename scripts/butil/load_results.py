...
def load_scores(evalType, tracker, testname, RESULT_SRC):
    ...
    for attrName in attrNames:
        ...
        j = json.loads(string)
        attr = Score(**j)
        attr.successRateList = map(lambda o:o, attr.successRateList)
        attrs.append(attr)
        attrs.sort()
    return attrs


def load_scores_madonghomg(evalType, tracker, testname, RESULT_SRC):
    resultSRC = RESULT_SRC.format(evalType)
    print 'Loading \'{0}\'...'.format(tracker)
    if tracker == 'KCFraw_gray' or tracker == 'KCFvgg':
        src = os.path.join(resultSRC, tracker + '/scores')
    else:
        return []
    attrNames = os.listdir(src)
