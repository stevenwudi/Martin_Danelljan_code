...
def plot_tracking_rect(seqname, frame, img_rgb, tracker, gtRect, wait_second=0.001):
    ...
    if tracker.feature_type == 'vgg' or tracker.feature_type == 'resnet50' or tracker.feature_type == 'vgg_rnn' or tracker.feature_type == 'cnn':
        ...
        if tracker.sub_sub_feature_type == 'adapted_lr_hdt':
            plt.title("Stability:\n %s" % (', '.join("{0:.4f}".format(x) for x in tracker.stability)))
    else:
...
def plot_tracking_result_multi(frame, img_rgb, result_list, gtRect, seqName,wait_second=0.5):
    from matplotlib.patches import Rectangle
    plt.figure(1)
    plt.clf()
    ...
    # plt.axis('off')
    # plt.savefig('tnnls_figures/alr_figures_bolt173.png', dpi=74, bbox_inches='tight')
