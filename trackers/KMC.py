...
class KMCTracker:
    def __init__(self, feature_type='multi_cnn',
        ...
        if self.sub_sub_feature_type == 'adapted_lr_hdt':
            ...
            self.stability = np.ones(shape=(len(self.y)))
            self.adaptation_rate = np.ones(shape=(self.acc_time)) * self.adaptation_rate_range[0]
            # store pre-computed cosine window, here is a multiscale CNN, here we have 5 layers cnn:
            self.loss = np.zeros(shape=(self.acc_time, len(self.y)))
            self.gamma = gamma
        ...
        if self.feature_type == 'multi_cnn':
            ...
            if self.sub_sub_feature_type == 'adapted_lr_hdt':
                ...
                if frame > self.acc_time:
                    ...
                    self.stability = np.exp(- (self.loss[loss_idx]-self.loss_mean)**2 / (2 * self.loss_std**2++ np.finfo(np.float32).eps))
                    ...
                    self.adaptation_rate = np.clip(self.stability*(self.adaptation_rate_range[0] - self.adaptation_rate_range[1]),
                                                   self.adaptation_rate_range[1], self.adaptation_rate_range[0])
                    self.adaptation_rate_scale = np.mean(np.clip(self.stability*(self.adaptation_rate_scale_range[0] - self.adaptation_rate_scale_range[1]),
                                                                 self.adaptation_rate_scale_range[1], self.adaptation_rate_scale_range[0]))
